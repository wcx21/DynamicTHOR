...
class DynamicProcTHORObjectNavTask(Task[Controller]):
    def __init__(
        self,
        controller: Controller,
        sensors: List[Sensor],
...
class DynamicProcTHORObjectNavTestTaskSampler(ProcTHORObjectNavTaskSampler):
    """Works with PRIOR's object-nav-eval tasks."""
    ...
    def next_task2(
        self, env, task, task_id='0_0', static=False, random=False, random_object_pose=None
    ) -> Optional[ProcTHORObjectNavTask]:

        # difficulty = {"difficulty": ep["difficulty"]} if "difficulty" in ep else {}
        event = env.set_task_setting(task, static, random, random_object_pose)
        # if random:
        #     env.random_put()
        self._last_sampled_task = DynamicProcTHORObjectNavTask(
            visualize=self.episode_index in self.epids_to_visualize,
            controller=env,
            sensors=self.args.sensors,
            max_steps=self.args.max_steps,
            ...
            task_info={
                "mode": self.args.mode,
                "house_name": 'procthor',
                "sceneDataset": "Dynamic",
                "target_object_ids": [task['target_object']],
                "object_type": task['target_object_type'],
                "mirrored": False,
                "id": task_id,
            },
        )
